...
part 'fetch_post_repository.g.dart';

@riverpod
class FetchPostRepository extends _$FetchPostRepository {
  @override
  ...
  Future<Result<List<Posts>, Exception>> getNearbyFromInitial({
    ...
    try {
      final posts = List<Posts>.of(
        await ref.read(mapRepositoryProvider.future),
      );
      // 初期投稿と同一座標を起点に距離昇順
      posts.sort((a, b) {
...
Future<List<Map<String, dynamic>>> profileRepository(
  ...
  return ref.read(fetchPostServiceProvider.notifier).getPostsFromUser(userId);
}
